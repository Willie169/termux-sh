apt update --allow-releaseinfo-change -y && apt upgrade -y && apt install automake bash build-essential bzip2 clang cmake command-not-found curl dbus file gdb git golang grep libboost-all-dev libssl-dev iproute2 jq make maven mc nano neovim nodejs openjdk-11-jdk openssh-client openssh-server openssl perl procps python3-all-dev python3-venv ruby tar tmux vim wget zsh -y
