...
PKG='alsa-utils aria2 autoconf automake bash bc bison build-essential bzip2 chromium clang cmake command-not-found curl dbus debootstrap dnsutils dpkg fastfetch fdroidcl ffmpeg file firefox flex fontconfig fontconfig-utils freetype gdb geckodriver gh ghostscript git glab-cli gnucobol gnupg golang gperf grep gtkwave inkscape iproute2 iverilog jpegoptim jq libheif-progs libzmq llvm make matplotlib maven mc nano ncurses-utils neovim net-tools ngspice ninja nodejs openjdk-17 openjdk-21 openssh openssh-sftp-server openssl openssl-tool optipng perl poppler postgresql procps proot proot-distro pulseaudio pv python python-ensurepip-wheels python-numpy python-pandas python-pip python-scipy ruby rust tar termux-am termux-am-socket termux-api termux-auth termux-exec termux-keyring termux-licenses termux-tools tmux tor torsocks tree unrar uuid-utils valgrind vim wget which xmlstarlet yarn zsh'
XFCE=1
ANDROID=1
VIMRC=1
