...
PIPX='cmake-language-server gh2md meson pylatexenc'
GO=''
ANTLR=1
PLANTUML=1
TERMUX='termux'
...
perl -0777 -pe 's/(###############\n# Extra keys\n###############\n)/$1\nextra-keys = \[ \[ \{ key: ESC, popup: ":wq\\n" \}, \{ key: "EOF", popup: ":q\\n" \}, \{ key: "\`", popup: "\`\`\`" \}, \{ key: "Z", popup: \{ macro: "CTRL r", display: "CTRL r" \} \}, \{ key: ":%s\/", popup: \{ macro: "CTRL t", display: "CTRL t" \} \}, \{ key: ":V", popup: \{ macro: "CTRL 1", display: "CTRL 1" \} \}, \{ key: "d\\\$", popup: \{ macro: "CTRL 2", display: "CTRL 2" \} \}, \{ key: "yG", popup: "y\\\$" \} \], \[ \{ key: DRAWER, popup: "±" \}, \{ key: KEYBOARD, popup: "^" \}, \{ key: "\/", popup: "\\\\\\\\" \}, \{ key: DEL, popup: BKSP \}, \{ key: HOME, popup: "clear" \}, \{ key: UP, popup: \{ macro: "CTRL UP", display: "Up" \} \}, \{ key: END, popup: ENTER \}, \{ key: PGUP, popup: "gg" \} \], \[ \{ key: TAB, popup: \{ macro: "CTRL d", display: "CTRL d" \} \}, \{ key: "~", popup: \{ macro: "CTRL \\\\\\\\", display: "CTRL \\\\\\\\" \} \}, \{ key: CTRL, popup: \{ macro: "CTRL c", display: "CTRL c" \} \}, \{ key: ALT, popup: \{ macro: "CTRL z", display: "CTRL z" \} \}, \{ key: LEFT, popup: \{ macro: "CTRL LEFT", display: "Left" \} \}, \{ key: DOWN, popup: \{ macro: "CTRL DOWN", display: "Down" \} \}, \{ key: RIGHT, popup: \{ macro: "CTRL RIGHT", display: "Right" \} \}, \{ key: PGDN, popup: "G" \} \] \]\n/s' ~/.termux/termux.properties > tmp
...
[ -n "$UBUNTU" ] && [ "$UBUNTUINSTALL" -ne 0 ] && cp ~/termux-sh/ubuntu-debian.sh "$PDROOTFS/$UBUNTU/root/" && echo './ubuntu-debian.sh' | proot-distro login $UBUNTU --isolated --fix-low-ports --shared-tmp --no-arch-warning
...
[ -n "$DEBIAN" ] && [ "$DEBIANINSTALL" -ne 0 ] && cp ~/termux-sh/ubuntu-debian.sh "$PDROOTFS/$DEBIAN/root/" && echo './ubuntu-debian.sh' | proot-distro login $DEBIAN --isolated --fix-low-ports --shared-tmp --no-arch-warning
...
[ -n "$UBUNTUBOX" ] && [ "$UBUNTUBOXINSTALL" -ne 0 ] && cp ~/termux-sh/box64-wine64-winetricks.sh "$PDROOTFS/$UBUNTUBOX/root/" && echo './box64-wine64-winetricks.sh' | proot-distro login $UBUNTUBOX --isolated --fix-low-ports --shared-tmp --no-arch-warning
...
[ -n "$DEBIANBOX" ] && [ "$DEBIANBOXINSTALL" -ne 0 ] && cp ~/termux-sh/box64-wine64-winetricks.sh "$PDROOTFS/$DEBIANBOX/root/" && echo './box64-wine64-winetricks.sh' | proot-distro login $DEBIANBOX --isolated --fix-low-ports --shared-tmp --no-arch-warning
