...
PKG='2048-c alsa-utils aria2 automake bash bc binutils bison broot build-essential busybox bzip2 ca-certificates calcurse clang cmake command-not-found cowsay cronie curl dbus debootstrap dnsutils dpkg dust exiftool fastfetch fd ffmpeg file flex fortune fzf gdb gh ghostscript git git-sizer glab-cli glow gnupg golang gopls gperf grep gzip hugo hyperfine inkscape inxi iproute2 jadx jpegoptim jq lazygit lftp libavif libheif-progs libjxl-progs libwebp libxml2 libxslt lsd luajit lzip make mandoc matplotlib maven mediainfo mesa-demos mesa-vulkan-icd-freedreno mesa-zink mktorrent mplayer mpv nano net-tools netcat-openbsd ngspice ninja nmap nodejs-lts npm octave opencc-tools openjdk-21 openssh openssl-tool optipng p7zip pdftk perl plantuml poppler procs proot proot-distro pulseaudio pv pwgen python python-ensurepip-wheels python-numpy python-pandas python-pip python-scipy python-trash-cli qalc qemu-user-x86-64 qpdf ripgrep ruby rust scrcpy shellcheck shfmt socat sqlite strace stylua tar termux-am termux-am-socket termux-api termux-auth termux-exec termux-keyring termux-services termux-tools termux-x11-nightly tigervnc tmux tor torsocks traceroute transmission tree tsocks unar unrar uuid-utils uv vgmstream vim virglrenderer-mesa-zink w3m wget wget2 which xfce4 xmlstarlet xz-utils yazi yq zip zoxide zsh'
IMG2PDF=1
GITLFS=1
GITDELTA=1
YTDLP=1
...
DEBIAN_FRONTEND=noninteractive pkg install busybox ca-certificates coreutils curl file git gzip jq perl proot proot-distro pulseaudio tar termux-api termux-tools wget which xz-utils zip -y -o Dpkg::Options::="--force-confnew" -o Dpkg::Options::="--force-overwrite"
