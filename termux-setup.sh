...
[ -n "$PKG" ] && DEBIAN_FRONTEND=noninteractive apt install $PKG -y
...
DEBIAN_FRONTEND=noninteractive apt install git-delta -y
...
if [ "$YTDLP" -ne 0 ]; then
DEBIAN_FRONTEND=noninteractive apt install deno -y
...
if [ "$PHICE" -ne 0 ]; then
DEBIAN_FRONTEND=noninteractive apt install libxml2 libxslt rust uv -y
