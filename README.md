...
Follow the screen guide to complete it. If you see "termux-setup-all.sh finished" on screen, run `exit`. If the `~$` doesn't show up, pull down the notification bar, and tap `exit` on the notification of Termux. Now, you can restart Termux and enjoy it.
