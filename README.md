...
This repository contains Shell scripts for [Termux](https://github.com/termux/termux-app) including setup automation, shortcuts, and installations and configurations of development tools and emulation environments.
...
: approximately 22.97 GB.</li>
<li><code>UBUNTUINSTALL=1</code> or <code>DEBIANINSTALL=1</code>: approximately 27.93 GB.</li>
</ul></li>
...
2. **Termux packages installation**: Installs Termux packages set in `$PKG`. Note that you need to download and install APK from <https://github.com/termux/termux-x11> release to use Termux:X11.
2. **img2pdf**: Installs img2pdf if not `$IMG2PDF=0`.
2. **Git LFS**: Installs Git LFS if not `$GITLFS=0`.
...
- [`ubuntu-debian.sh`](ubuntu-debian.sh) (Only Ubuntu >= 24.04 and Debian >= 13 is supported. Running in Ubuntu is more recommended than in Debian since in Debian, some tools are not supported and some tools are of older versions.): Configures PulseAudio and VirGL (according to my [**termux-x11-virgl-gpu-acceleration**](https://github.com/Willie169/termux-x11-virgl-gpu-acceleration) repo and with variables exported in `~/.bashrc`) and installs development tools, runtime environments, and utilities for C, C++, Python3, Java 21, 
  Rust, Go, Perl, LuaJIT, Aptitude, trash-cli, Git LFS, git-sizer, GitHub CLI, GitLab CLI, GVim, OpenSSL, OpenSSH, JQ, FFMPEG, Pandoc, TeX Live (via regular installation instead of APT for unrestricted `tlmgr` and updates, can be updated with `update_texlive`), Maven, Zsh, XFCE desktop environment, checkinstall, iproute2, 
  net-tools, traceroute, Nmap, Node.js LTS (via NVM), npm, Yarn, Deno, http-server, yt-dlp, Lazygit, Homebrew, Glow, bat, fd, ncdu, dust, fzf, Delta, ripgrep (rg), Yazi, zoxide, Miniforge, uv, autopep8, gallery-dl, procs, tldr, xmljson, JADX, Apktool, broot, bottom (btm), hyperfine, lsd, Qalculate! CLI (qalc), 2048, Cargo B(inary)Install (cargo-binstall), BleachBit, 
  fdupes, bnkextr, vgmstream, Hugo, gh2md, img2pdf, English, Chinese - Simplified, Chinese - Simplified (vertical), Chinese - Traditional, and Chinese - Traditional (vertical) Tesseract OCR, OCRmyPDF, LibreOffice no GUI, LibreTranslate, Jupyter Notebook, JupyterLab, Jupytext, Meson, pylatexenc, The Unarchiver (unar), RARLAB UnRAR, Verilator, Ngspice, jpegoptim, optipng, exiftool, libavif, libjxl, libheif, LibWebP, ImageMagick, Inkscape, Poppler, qpdf, PDFtk, Ghostscript, Bookletimposer, Dungeon Crawl Stone Soup, NetHack, Brogue CE, 
  Audacity, w3m, XMLStarlet, GTKWave, Octave, rTorrent, mktorrent, OpenCC, libqrencode, barcode, ZBar, XFCE desktop environment (can be started with Termux:X11 by running `termux-x11 :0 &`, logging into Proot-distro with `--shared-tmp` option, and running `export DISPLAY=:0` and `xfce` inside, where `0` can be replaced with other numbers), fix for stuck `elementary-xfce-icon-theme` from my [**fix-elementary-xfce-icon-theme**](https://github.com/Willie169/fix-elementary-xfce-icon-theme) repo, PlantUML, SQLite 3, yosys – Yosys Open SYnthesis Suite, 
...
- [`proot-ubuntu.sh`](DOTshortcuts/proot-ubuntu.sh), [`proot-debian.sh`](DOTshortcuts/proot-debian.sh): Boot those Proot-distro environments installed in [`termux-setup.sh`](termux-setup.sh) respectively.
...
- [`persistent-notifications-rish.sh`](persistent-notifications-rish.sh): On Android 14+, let persistent notifications posted by all apps of all users not be dismissible through the UI (i.e. behave as they did prior to Android 14) (run in ADB shell) assuming that interactive ADB shell can be accessed with `rish`. See my [**Samsung-Android-Debloat-List**](https://github.com/Willie169/Samsung-Android-Debloat-List) repo for more information.
- [`persistent-notifications-adb.sh`](persistent-notifications-adb.sh): On Android 14+, let persistent notifications posted by all apps of all users not be dismissible through the UI (i.e. behave as they did prior to Android 14) (run in ADB shell) assuming that ADB is connected with <code>adb shell</code> available. Arguments passed to it will be passed to ADB. See my [**Samsung-Android-Debloat-List**](https://github.com/Willie169/Samsung-Android-Debloat-List) repo for more information.
...
- [`disable-phantom-process-killer-adb.sh`](disable-phantom-process-killer-adb.sh): Disable phantom process killer to resolve `Process completed (signal 9) - press Enter` error assuming that ADB is connected with <code>adb shell</code> available. Arguments passed to it will be passed to ADB. See <https://willie169.github.io/Android-Non-Root/#process-completed-signal-9---press-enter-error> for more information. 
