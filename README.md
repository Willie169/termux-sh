...
- **[qemu-alpine-setup.sh](qemu-alpine-setup.sh)**: Fetches the `alpine-virt-3.21.0-x86_64.iso` in `~/alpine` and create QEMU image `alpine.img` on it. Run `setup-alpine` after QEMU VM booted to set it up.
