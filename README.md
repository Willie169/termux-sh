...
2. **Python packages**: Installs pip packages set in `PIPINSTALL` in it if `PIPINSTALL` is not empty. The default ones are `jupyter meson numpy pandas pipx pydub selenium setuptools sympy`.
