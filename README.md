...
<li>Approximately 37.6 GB more for <code>UBUNTUINSTALL=1</code>.</li>
<li>Approximately 37.6 GB more for <code>DEBIANINSTALL=1</code>.</li>
<li>Approximately 58.4 GB in total for the default configuration with <code>XFCE=1</code>, <code>ANDROID=1</code>, and
...
- [`ubuntu-debian.sh`](ubuntu-debian.sh): Configures PulseAudio and VirGL (according to my <https://github.com/Willie169/termux-x11-virgl-gpu-acceleration> repo and with variables exported in `~/.bashrc`) and installs development tools, runtime environments, and utilities for C, C++, Python3, Java 21, 
  ...
  net-tools, Nmap, Alien, Node.js and NPM (via APT), NPM packages `jsdom markdown-toc marked marked-gfm-heading-id node-html-markdown showdown` locally in `~`, http-server, OpenCode, Codex, Lazygit, Bun, Homebrew, Glow, bat, fd, fzf, Delta, Tennis, ripgrep (rg), starship, Yazi, zoxide, Nerd Fonts, eza, Miniforge, pipx, uv, Poetry, MarkItDown, 
