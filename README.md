...
- **[apline-docker-setup.sh](apline-docker-setup.sh)**: After running [qemu-alpine-setup.sh](qemu-alpine-setup.sh) on Termux and then `setup-alpine` in QEMU VM and then `poweroff`, run [qemu-apline.sh](DOTshortcuts/qemu-alpine.sh) on Termux to boot QEMU VM again, and then run this script in QEMU VM to install Docker.
