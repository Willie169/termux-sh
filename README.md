...
2. **Proot-distro Termux environment with minimal configurations**: Installs an Termux proot-distro environment with alias `$TERMUX` and add boot script `proot-$TERMUX.sh` into home directory `~` and `~/.shortcuts` folder (for **Termux:Widget**) if `$TERMUX` is not empty string, and runs [`termux-proot.sh`](termux-proot.sh) in it if `$TERMUX` is not empty string and `$TERMUXINSTALL` is not `0`. See [Invoked VM Setup Scripts](#invoked-vm-setup-scripts) section for more information.
...
- [termux-proot.sh](termux-proot.sh): Copies `~/.bashrc.d` and `~/.bashrc` from my [**bashrc**](https://github.com/Willie169/bashrc) repo on Termux Proot environment.
