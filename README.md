...
-  [qemu-alpine.sh](DOTshortcuts/qemu-alpine.sh): Start a headless QEMU VM running Alpine Linux 3.21 with 1 GB RAM, 2 CPU cores, and SSH port forwarding (host: 2222 → guest: 22).
...
- **[qemu-debian-download.sh](qemu-debian-download.sh)**: Fetches the QEMU Debian Bookworm AMD64 QCOW2 virtual disk image
- **[qemu-alpine-setup.sh](qemu-alpine-setup.sh)**: Fetches the `alpine-virt-3.21.0-x86_64.iso` in `~/alpine` and create QEMU image `alpine.img` on it. Run `setup-alpine` after QEMU VM booted to set it up, and then reboot it after it.
- **[alpine-docker-setup.sh](alpine-docker-setup.sh)**: After running [qemu-alpine-setup.sh](qemu-alpine-setup.sh) on Termux and then `setup-alpine` in QEMU VM and then `poweroff`, run [qemu-alpine.sh](DOTshortcuts/qemu-alpine.sh) on Termux to boot QEMU Alpine VM again, and then run this script in QEMU VM to install Docker.
...
- Expand VMs, and development tools script support.
