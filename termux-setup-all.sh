...
tmux send-keys 'cd ~ && mkdir debian2 && cd debian2 && wget https://raw.githubusercontent.com/AndronixApp/AndronixOrigin/master/Installer/Debian/debian-xfce.sh -O debian-xfce.sh && chmod +x debian-xfce.sh && echo \'1 && chmod +x debian2-setup.sh && ./debian2-setup.sh && rm debian2-setup.sh && vncserver-stop && trap "exit" INT TERM\' bash debian-xfce.sh' C-m
tmux select-pane -t 2
tmux send-keys 'while true; do if [ -d ~/debian2/debian-fs/root ]; then cp ~/termux-sh/debian2-setup.sh ~/debian2/debian-fs/root break fi done' C-m
